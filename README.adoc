...
[cols="^,^,^,^", options="header"]
.java.time manipulation methods (Requires `EnhancedJava8ObjectWrapper`)
|===
| java.time class | methods | comment | example

|LocalDate / LocalDateTime / LocalTime
|`plusSeconds(<Long>), plusMinutes(<Long>), plusDays(<Long>), plusWeeks(<Long>), plusMonths(<Long>), plusYears(<Long>)`
|Can create a new Temporal object with specified time difference from the original object.
